...
def extract_specific_content(url):
     try:
          response=requests.get(url)
          response.raise_for_status() #raise_for_status and status code : difference 

          soup=BeautifulSoup(response.text,'html.parser')
          #skip extraction for social media URLs
          if is_social_media(url):
               print(f'skipping {url} because it is a social media link.')
               ...
               publication_data_element=soup.find('div',class_='updated-time')
               ...
               publication_date_raw=publication_date_element.text.strip() if publication_date_element else'Publication not found'
               publication_date_element=publication_date_raw.replace('published at:','').strip()
...
def save_to_csv(data,csv_file_path):
     with open(csv_file_path,'a',newline='',encoding='utf-8') as csv_file:
          ...
          csv_writer.writerow(['Heading','Author','Publication_date','Source','content','Url','Category'])
...
def main():
     ...
     link=pd.read_csv('datalinks.csv')
     for url in links:
          if not urlparse(url).scheme:
               ...
               if urlparse(url).netloc and not is_social_media(url):
                    ...
                    if heading is not None and author is not None and content is not None:
                         ...
                         print(f'Data for {url}:\n Heading:{heading}\nAuthor:{author}\nPublicationDate:{publication_date}\nSource:Kathmandu-Post,content{content}\n link:{link}\n category:{category}')
                         save_to_csv(data_to_save,csv_file_path)
                         print(f'data saved for {url}')
                    else:
                         print(f'Invalid URL format for social media links{url}') 
